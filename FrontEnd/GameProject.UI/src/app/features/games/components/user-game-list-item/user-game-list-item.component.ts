...
export class UserGameListItemComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(this.gameStatuses)
    console.log(GameStatusToLabelMapping[this.status]);
  ...
  changeGameStatus() {
    const log: ChangeGameStatusRequest = {
      game: {
        platforms: [],
        name: this.game.name,
    ...
    this.userGameListService.changeGameStatusInUserList(log
    )
      .subscribe({
        next: _ => {
          this.authenticationService.currentUser$.pipe(take(1))
